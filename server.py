import grpc
from concurrent import futures
import json
import time
import banks_pb2_grpc
from branch import Branch
import sys

def server(input_file):
    
    # Load the input file for every server instance
    with open(input_file) as f:
        database = json.load(f)

    # Gather branch selections for servers
    branches = [item for item in database if item["type"] == "branch"]

    # Create Server List data struct to hold servers
    servers = []

    # for each branch instance create a server and add to servers list
    for branch_instance in branches:
        branch_id = branch_instance["id"]
        port = 50050 + branch_id
        branch_instance["port"] = port

        server_instance = grpc.server(futures.ThreadPoolExecutor(max_workers=10))

        banks_pb2_grpc.add_BranchServiceServicer_to_server(
            Branch(branch_id, branch_instance["balance"], branches),
            server_instance
        )

        server_instance.add_insecure_port(f"[::]:{port}")
        server_instance.start()
        servers.append(server_instance)

        print(f"[Server] Branch_instance {branch_id} active within port_number {port}")


    # keep the servers awake
    try:
        while True:
            time.sleep(50000)
    except KeyboardInterrupt:
        for s in servers:
            s.stop(0)
        print("[Server] Turning off")

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("python server.py input_file.json")
    else:
        server(sys.argv[1])
